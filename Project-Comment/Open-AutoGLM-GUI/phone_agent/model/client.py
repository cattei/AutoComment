...
class ModelConfig:
    ...
    # License认证Header（BS架构：每次请求都带Key+设备指纹）
...
class ModelClient:
    ...
    Supports both direct API access and proxy mode (BS architecture):
    - Direct: base_url points to AI provider, api_key is real API key
    - Proxy: base_url points to license server, api_key is user license key,
             auth_headers carries device fingerprint for verification
    ...
    def __init__(self, config: ModelConfig | None = None):
        ...
        # 构造OpenAI客户端，支持自定义Header（License认证）
        default_headers = self.config.auth_headers if self.config.auth_headers else None
        self.client = OpenAI(
